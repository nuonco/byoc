...
AZURE | {{ $location }} | {{ dig "resource_group_name" "rg-unknown" .nuon.install_stack.outputs }}
{{ else }}
AZURE | xx-region | rg-unknown
{{ end }}
  </small>

<small>[Dashboard](https://app.{{
...
> [!NOTE]
> This app is an Azure-first BYOC configuration.
> Container images follow the Restate Azure pattern: source via `container_image` components and deploy using ACR-prefixed runtime refs.
...
- [Accessing the AKS Cluster](#accessingtheakscluster)
- [Secrets](#secrets)
...
configure install DNS delegation. This second feature is optional but, at the time of writing, a default value must be
provided.
...
When an install is created, DNS records and nameserver delegation details become available below.
...
##### Nuon DNS Domain Input

| Attribute   | Value                               |
| ----------- | ----------------------------------- |
| Domain Name | {{ .nuon.inputs.inputs.nuon_dns_domain }} |

Additional Documentation

- Configure NS delegation with your DNS provider using the nameservers above.
...
| Nuon DNS Domain | `{{ .nuon.inputs.inputs.nuon_dns_domain }}` | The domain used to provision domains for installs managed by this BYOC Nuon Install. |

### Update Secrets

When provisioning the install stack, you will need to provide the following secrets.
...
The github app PEM key must be base64 encoded. Encoding the PEM key before pasting it in, and decoding it later when
it's read, preserves the newlines in the text.
...
| Install Stack QuickLink URL      | [{{ .nuon.install_stack.quick_link_url }}]({{ .nuon.install_stack.quick_link_url }})                                                                                                                                                                                                                                           |
| Install Stack Template URL       | [{{ .nuon.install_stack.template_url }}]({{ .nuon.install_stack.template_url }})                                                                                                                                                                                                                                               |
...
1. Ensure your Azure principal has access to the cluster resource group.
2. Get AKS credentials with the following command.

<pre>
az aks get-credentials \
    --resource-group {{ .nuon.install_stack.outputs.resource_group_name }} \
    --name {{ dig "outputs" "cluster" "name" "$cluster_name" .nuon.sandbox }}
</pre>

## Secrets

The following secrets are created in the install stack and then synced into the cluster.
