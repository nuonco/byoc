...
#
# The ctl-api tables are owned by the ctl-api IAM database role, so we query as
# that role via Cloud SQL IAM auth: the runner impersonates the ctl-api SA
# (roles/iam.serviceAccountTokenCreator, granted by ctl_api_wi) to mint a
# sqlservice.login-scoped token, used as the DB password over SSL. See
# query-builds.sh for the full rationale.
#
# Env vars:
...
#   DB_NAME / DB_PORT / DB_ADDR  (see query-builds.sh)
...
if [[ -z "$db_addr" ]]; then
  echo "[query build] discovering db connection from ctl-api deployment"
...
if [[ -z "$db_addr" ]]; then
  echo "[query build] ERROR: could not determine DB_ADDR; set it explicitly" >&2
  exit 1
fi

echo "[query build] db_addr=$db_addr"
...
# Mint a Cloud SQL login token AS the ctl-api service account (Cloud SQL IAM
# auth). See query-builds.sh for the full rationale.
echo "[query build] minting a Cloud SQL login token as ${CTL_API_SA_EMAIL}"
db_token=$(gcloud auth print-access-token \
  --impersonate-service-account="$CTL_API_SA_EMAIL" \
  --scopes=https://www.googleapis.com/auth/sqlservice.login)
if [[ -z "$db_token" ]]; then
  echo "[query build] ERROR: failed to mint a login token as $CTL_API_SA_EMAIL." >&2
  echo "[query build] (does the runner SA have roles/iam.serviceAccountTokenCreator on it? redeploy ctl_api_wi.)" >&2
  exit 1
fi
