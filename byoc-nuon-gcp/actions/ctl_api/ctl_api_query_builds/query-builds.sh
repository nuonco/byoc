...
#
# The ctl-api tables (component_builds, etc.) are owned by the ctl-api IAM
...
# table component_builds". We authenticate as ctl-api by impersonating its SA
...
# The token is scoped to sqlservice.login, which Cloud SQL requires for direct
# (non-proxy) IAM auth.
echo "[query builds] minting a Cloud SQL login token as ${CTL_API_SA_EMAIL}"
db_token=$(gcloud auth print-access-token \
  --impersonate-service-account="$CTL_API_SA_EMAIL" \
  --scopes=https://www.googleapis.com/auth/sqlservice.login)
if [[ -z "$db_token" ]]; then
  echo "[query builds] ERROR: failed to mint a login token as $CTL_API_SA_EMAIL." >&2
  echo "[query builds] (does the runner SA have roles/iam.serviceAccountTokenCreator on it? redeploy ctl_api_wi.)" >&2
  exit 1
fi
