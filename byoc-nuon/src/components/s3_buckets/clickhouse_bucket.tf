...
#
locals {
  bucket_name = "${var.install_id}-nuon-clickhouse"
...
resource "aws_iam_role" "clickhouse_role" {
  name               = "${var.install_id}-nuon-clickhouse-role"
  assume_role_policy = data.aws_iam_policy_document.clickhouse_trust_policy.json

  # bucket access policy
  inline_policy {
    name   = "${var.install_id}-nuon-clickhouse-role-inline-bucket-access-policy"
